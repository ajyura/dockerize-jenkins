...
The four Docker images that should be created are:
  ...
  ##### Instructions for jenkins-master
   ...
   4. ***If you wish to map jenkins to a volume on the host, note the path to that volume.*** In order to port Jenkins configurations, job data, and credentials from another instance of Jenkins, the host volume must be a copy of the JENKINS_HOME directory from the original Jenkins instance.
